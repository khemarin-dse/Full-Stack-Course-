...
import { getGoalMeta, getCategoryMeta } from '../utils/categories'
import api from '../utils/api'

export default function GoalDetail() {
  const { id } = useParams()
  const navigate = useNavigate()
  const { goals, fetchGoals, deleteGoal } = useFinance()
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[20px] font-semibold">{goal.name}</h1>
            {goal.deadline && <p className="text-[13px] text-[#6B6882]">Deadline: {new Date(goal.deadline).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</p>}
          </div>
        </div>
        <div className="flex gap-2">
          <Btn variant="outline" size="sm" onClick={() => setShowEdit(true)}><i className="ti ti-pencil text-[13px]"></i> Edit</Btn>
          <Btn variant="danger-outline" size="sm" onClick={handleDelete}><i className="ti ti-trash text-[13px]"></i> Delete</Btn>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Card className="p-5 flex flex-col gap-4">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-[12px] flex items-center justify-center" style={{ background: meta.bg }}>
              <i className={`ti ${meta.icon} text-[24px]`} style={{ color: meta.color }}></i>
            </div>
            <div>
              <div className="text-[18px] font-bold">{goal.name}</div>
              {goal.note && <div className="text-[12px] text-[#6B6882] mt-0.5">{goal.note}</div>}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex justify-between text-[12px]">
              <span className="font-semibold">${Number(goal.saved_amount).toLocaleString()} saved</span>
              <span className="text-[#6B6882]">${Number(goal.target_amount).toLocaleString()} target</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              { label: 'Needed / month', value: neededPerMonth ? `$${neededPerMonth.toLocaleString()}` : '—', highlight: true },
              { label: 'Avg saved / month', value: `$${avgSaved.toLocaleString()}`, green: true },
            ].map(s => (
              <div key={s.label} className="bg-[#F8F8FB] border border-[#E4E2F0] rounded-[10px] p-3">
                ...
                <div className={`text-[18px] font-semibold ${s.highlight ? 'text-primary' : s.green ? 'text-[#0F6E56]' : 'text-[#1A1730]'}`}>{s.value}</div>
              </div>
            ))}
          </div>
          ...
          {neededPerMonth && neededPerMonth > avgSaved && (
            <div className="bg-[#FAEEDA] rounded-[10px] p-3 flex items-start gap-2">
              ...
              <span className="text-[12px] text-[#633806]">
                At your current pace you need <strong>${neededPerMonth.toLocaleString()}/month</strong> to hit your goal. Try saving a bit more!
              </span>
            </div>
          )}
        ...
        <Card className="p-5">
                ...
                return (
                  <div key={i} className="flex items-center gap-3 p-2.5 bg-[#F8F8FB] rounded-[10px]">
                    ...
                    <div className="flex-1">
                      <div className="text-[13px] font-medium">{h.description || 'Savings deposit'}</div>
                      <div className="text-[11px] text-[#6B6882]">
                        {new Date(h.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                        {' · '}{isManual ? 'manual update' : 'via transaction'}
                      </div>
                    </div>
                    <span className="text-[13px] font-semibold text-[#0F6E56]">+${Number(h.amount).toLocaleString()}</span>
                  </div>
                )
              })}
            </div>
          ...
          <div className="border-t border-[#E4E2F0] pt-3 mt-3 flex justify-between text-[13px]">
            ...
            <span className="font-semibold text-[#0F6E56]">${Number(goal.saved_amount).toLocaleString()}</span>
          </div>
        </Card>
      </div>
